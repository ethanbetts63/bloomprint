...
from partners.models import BusinessAccount, DiscountCode, Commission
...
class BusinessAccountDashboardSerializer(serializers.ModelSerializer):
    ...
    def get_discount_code_summary(self, obj):
        if obj.account_type != 'affiliate':
            return {'active_codes': 0, 'total_uses': 0}
        return {
            'active_codes': obj.discount_codes.filter(is_active=True).count(),
            'total_uses': DiscountCode.objects.filter(business_account=obj).aggregate(total=Count('usages'))['total'] or 0,
        }
