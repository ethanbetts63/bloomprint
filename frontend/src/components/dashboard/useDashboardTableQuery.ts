...
export function useDashboardTableQuery(defaultSort: SortState = { field: 'created_at', dir: 'desc' }) {
  ...
  const setKind = (value: string) => update({ type: value, page: null });
  ...
  const clear = () => { setDraftSearch(null); update({ status: null, type: null, search: null, ordering: null, page: null }); };
