...
class TestGuestCheckoutClaim:
    ...
    def test_claiming_an_existing_staff_email_does_not_touch_that_account(self):
        ...
        staff = User.objects.create_user(
            username='boss', email='boss@bloomprint.com.au', is_staff=True
        )
        ...
        response = client.post(
            CLAIM_URL,
            {'email': 'boss@bloomprint.com.au', 'first_name': 'Not', 'last_name': 'Boss'},
            format='json',
        )
        assert response.status_code == 200, response.data

        order.refresh_from_db()
        assert order.customer_email == 'boss@bloomprint.com.au'
